...
def save_image_to_media(image_url: str, user_id: str) -> tuple[str, str]:
    """Downloads image to MEDIA_ROOT/<user_id>/<uuid>.<ext>.
    ...
    fname = f"initial_{uuid.uuid4().hex}{ext}"
...
# --- routes: account/stripe (unchanged) --------------------------------------
@app.get("/api/me")
def me():
...
def finalize_influencer():
    ...
    # (Optional) also record the initial image in images table
